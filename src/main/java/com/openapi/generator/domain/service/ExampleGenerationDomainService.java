...
public class ExampleGenerationDomainService {
    ...
    private static final List<String> DOMAINS = List.of(
        "example.com", "test.org", "demo.io", "sample.net", "mock.dev"
    );
    ...
    private String generateStringValue(String name, String format, int index) {
        return switch (format) {
            case "date" -> '"' + LocalDate.now().minusDays(index).format(DateTimeFormatter.ISO_DATE) + '"';
            case "date-time", "datetime" -> '"' + LocalDateTime.now().minusHours(index)
    ...
    private String generateStringByName(String name, int index) {
        if (name.contains("email") || name.contains("mail")) {
        ...
        } else if (name.contains("firstname") || (name.contains("first") && name.contains("name"))
                || name.contains("givenname")) {
            return '"' + FIRST_NAMES.get(index % FIRST_NAMES.size()) + '"';
        } else if (name.contains("lastname") || name.contains("familyname") || name.contains("surname")
            ...
            return '"' + "+1-555-" + String.format("%03d", index + 100)
        ...
        } else if (name.contains("description") || name.contains("text") || name.contains("content")
                || name.contains("body")) {
            return '"' + "Sample " + name + " " + (index + 1) + " - This is a generated example value." + '"';
